...
<body>
	...
	이유는 요청처리하는 자바코드와 화면을 구성하는 html 태그를 분리할 때
	jsp 파일을 태그형식으로만 작성하기 위함입니다.
	</p>
	<p>태그 라이브러리는 맨 위 지시자에서 정의하고 사용합니다. EL 기호는 애트리뷰트 값만
		출력할 수 있다.
	</p>
	<% String name="sana"; %>
	<hr>
	<h4>c: 으로 시작하는 코어 태그</h4>
	...
	<h5>name : <c:out value="${name }"/></h5> <!-- name는 자바변수. 출력안함 -->
	<!-- 3. 자바의 if문을 대체하는 태그입니다. -->
	<h5>c:if TEST</h5>
	<!-- 조건식을 test 속성에 작성 EL 기호안에 한다. -->
	<c:if test="${age <20 }">
...
<%
		...
		//만들어진 객체를 pageContext scope의 애트리뷰트로 저장합니다.
		//애트리뷰트 이름 customer, 값은 vo
		pageContext.setAttribute("customer", vo);
%>
	<c:out value="${customer }"></c:out>
	<ul>
		<!-- 객체변수명.getXXX() getter 메소드 사용.
			그러나 EL은 알아서 getter 동작을 함. 객체의 속성 이름을 정확히 작성
		 -->
